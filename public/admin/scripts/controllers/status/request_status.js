angular.module('dmt-back').controller('listItemRequestStatusController', function ($scope, $mdDialog, $mdEditDialog, page, $http, $filter,STATES) {
	var ctrl = this;
	ctrl.page = page;
	ctrl.STATES = STATES;
	...
	$scope.editField = function (event, item, field) {
			...
			if(item.id === ctrl.STATES.EVALUATION_REQUEST.PENDIENTE || 
				item.id === ctrl.STATES.EVALUATION_REQUEST.ERROR || 
				item.id === ctrl.STATES.EVALUATION_REQUEST.POR_ASIGNAR || 
				item.id === ctrl.STATES.EVALUATION_REQUEST.RECHAZADO || 
				item.id === ctrl.STATES.EVALUATION_REQUEST.CUMPLE || 
				item.id === ctrl.STATES.EVALUATION_REQUEST.NO_CUMPLE ){
				return;
			}
