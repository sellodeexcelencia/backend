angular.module('dmt-back').controller('listItemStatusController', 
function ($scope, $mdDialog, $mdEditDialog, page, $http, $filter, STATES) {
	var ctrl = this;
	ctrl.STATES = STATES
	ctrl.page = page;
	...
	$scope.editField = function (event, item, field) {
			...
			if(item.id === ctrl.STATES.SERVICE.NO_CUMPLE || item.id === ctrl.STATES.SERVICE.CUMPLE){
				return;
			}
